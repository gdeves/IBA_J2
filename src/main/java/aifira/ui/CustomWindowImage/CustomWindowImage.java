package aifira.ui.CustomWindowImage;
...
public void actionPerformed(ActionEvent e) {
    ...
    if (b == roiCalculation) {
        // ✅ Vérifier qu'il y a UNE ROI sélectionnée
        if (imp.getRoi() == null) {
            IJ.showMessage("Erreur", "Veuillez sélectionner une ROI sur l'image !");
            return;
        }
        
        // ✅ Ajouter la ROI au manager SI elle n'existe pas
        Roi currentRoi = imp.getRoi();
        manager.addRoi(currentRoi);  // ou manager.add(imp, currentRoi, manager.getCount()+1)
        
        // ✅ Calculer les spectres
        for (int index = 0; index < manager.getCount(); index++) {
            try {
                manager.select(index);
                String roiName = manager.getName(index);
                IJ.log("Calculating spectra from ROI: " + roiName);
                
                Spectra roiSpectra = stack[0].roiSpectra();
                
                if (roiSpectra == null) {
                    IJ.log("ERREUR: roiSpectra() a retourné null pour " + roiName);
                    continue;
                }
                
                roiSpectra.setFilename(roiSpectra.getPath(manager.getName(index)));
                roiSpectra.plot("Spectra from ROI: " + roiName, tr("Datafile") + " " + roiSpectra.getPath()).showVisible();
                roiSpectra.getADC().saveGupixSpectra(roiSpectra.getPath() + ".gup");
                roiSpectra = null;
                
            } catch (Exception ex) {
                IJ.log("ERREUR lors du calcul: " + ex.getMessage());
                ex.printStackTrace();
            }
        }
        System.gc();
