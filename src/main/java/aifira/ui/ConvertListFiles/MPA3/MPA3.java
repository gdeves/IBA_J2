package aifira.ui.ConvertListFiles.MPA3;
...
public final class MPA3{
    ...
    */
     ...
     * @param position index of the ADC which will contains the sum of the events
     */
    public void addToSum(int position, int sumPosition,int offset){
        //if it is the first ADC to add
        if  (mpa.size()==17){
            ADC initialADC=getADC(sumPosition);
            ADC adc2=getADC(position);
            for (int i=0;i<adc2.getNEvents();i++) {
                adc2.setE(i, adc2.getE(i)+offset);
                initialADC.addEvent(adc2.getEvent(i));
            }
            
        }
        else mpa.add(getADC(position));
    }    
    /**
